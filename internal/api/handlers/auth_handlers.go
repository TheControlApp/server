...
func (h *AuthHandlers) Login(c *gin.Context) {
	req, err := api.BindJSON[LoginRequest](c)
	if err != nil {
		api.Error(c, err)
		return
	}
	...
	api.JSON(c, http.StatusOK, api.AuthResponse{
...
func (h *AuthHandlers) Register(c *gin.Context) {
	req, err := api.BindJSON[RegisterRequest](c)
	if err != nil {
		api.Error(c, err)
		return
	}
	...
	api.JSON(c, http.StatusCreated, api.UserResponse{User: *user})
}
